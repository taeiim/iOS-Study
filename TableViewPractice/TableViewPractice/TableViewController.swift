...
class TableViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableView.dequeueReusableCell(withIdentifier: "ImageTableViewCell", for: indexPath) as? ImageTableViewCell else { return .init() }
        cell.indexLabel.text = items?[indexPath.row].owner.login
        
        let profileUrl = URL(string: items?[indexPath.row].owner.avatarURL ?? "")
        cell.imageView?.sd_setImage(with: profileUrl)
    ...
    override func viewDidLoad() {
        ...
        AF.request("https://api.github.com/search/repositories", method: .get, parameters: ["q":"androiddd"]).responseJSON { (response) in
